...
def load_sc_args(config: ModelConfig):  # noqa
    path = os.path.join(config.param_directory, config.name + "_scp.npy")
    return load_npy(path)


def save_sc_args(config: ModelConfig, args):  # noqa
    path = os.path.join(config.param_directory, config.name + "_scp.npy")
    np.save(path, args)
...
def get_datasets(c: ModelConfig) -> Tuple[ProteinDataset, Optional[ProteinDataset], Optional[ProteinDataset]]:
    dataset = lambda lst, nat, decoy, seq, samples=-1: ProteinDataset(
        model_list=lst,
        native_folder=nat,
        decoy_folder=decoy,
        seq_folder=seq,
