...
def _get_residue_ens(pose, device='cpu', dtype=torch.float32):
    ...
    total_eng = pyrosetta.bindings.energies.total_energies_array(pose.energies())
    ...
    return rosetta_to_2Dtensor(residue_total_ens, device=device, dtype=dtype), \
           rosetta_to_3Dtensor(residue_pair_ens, device=device, dtype=dtype), \
           torch.tensor(list(total_eng), dtype = torch.float32)  # noqa
...
class RosettaScore:
    ...
    def get_energies(self, seq, coords, device=None, dtype=None, pdb=None):
        ...
        res, pair, total = _get_residue_ens(pose, device=device, dtype=dtype)
        return res, pair, total
