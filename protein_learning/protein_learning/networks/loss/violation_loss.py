...
class InterResidueVDWRepulsiveLoss(nn.Module):
    ...
    def __init__(self, tol: float = 1e-1):  # noqa
        """
    ...
    def forward(
        ...
        atom_dists = safe_norm(to_rel_coord(rel_atom_coords), dim=0)
        ...
        per_atom_violations = torch.sum(torch.square(violations) * pair_mask, dim=-1) / 2
        if reduce:
            return torch.sum(per_atom_violations)
        return rearrange(per_atom_violations, "b (n a) -> b n a", n=n)
...
class BackboneAngleDeviationLoss(nn.Module):
    ...
    def forward(
        ...
        for idx, (o1, o2, o3) in enumerate(self.offsets):
            ...
            dev = F.relu(torch.square(cos_theta - ideal_angles) - self.tols[idx])
            loss = loss + masked_mean(dev, valid_mask, dim=-1)
